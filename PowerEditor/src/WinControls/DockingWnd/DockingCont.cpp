...
static LRESULT CALLBACK hookProcMouse(UINT nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (wParam)
    {
		case WM_MOUSEMOVE:
		case WM_NCMOUSEMOVE:
			::PostMessage(hWndServer, static_cast<UINT>(wParam), 0, 0);
			break;
		case WM_LBUTTONUP:
		case WM_NCLBUTTONUP:
			::PostMessage(hWndServer, static_cast<UINT>(wParam), 0, 0);
...
{
	return static_cast<int>(::SendMessage(_hContTab, TCM_GETCURSEL, 0, 0));
}
...
std::vector<tTbData*> DockingCont::getDataOfVisTb()
{
	...
	TCITEM				tcItem		= {0};
	int					iItemCnt	= static_cast<int>(::SendMessage(_hContTab, TCM_GETITEMCOUNT, 0, 0));
...
void DockingCont::drawCaptionItem(DRAWITEMSTRUCT *pDrawItemStruct)
{
	...
	UINT		length  	= static_cast<UINT>(_pszCaption.length());
...
LRESULT DockingCont::runProcTab(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (Message)
	{
		...
		case WM_LBUTTONUP:
		{
			...
			int iItem = static_cast<int>(::SendMessage(hwnd, TCM_HITTEST, 0, (LPARAM)&info));
		...
		case WM_MOUSEMOVE:
		{
            ...
            {
				int	iItemSel = static_cast<int>(::SendMessage(hwnd, TCM_GETCURSEL, 0, 0));
...
//
LRESULT CALLBACK DockingCont::run_dlgProc(UINT Message, WPARAM wParam, LPARAM lParam)
{
	switch (Message)
	{
		case WM_NCACTIVATE:
...
void DockingCont::onSize()
{
	...
	RECT	rc			= {0};
	RECT	rcTemp		= {0};
	UINT	iItemCnt	= static_cast<int>(::SendMessage(_hContTab, TCM_GETITEMCOUNT, 0, 0));
	UINT	iTabOff		= 0;
	...
	if (iItemCnt >= 1)
	{
		...
		UINT	iItemCnt = static_cast<UINT>(::SendMessage(_hContTab, TCM_GETITEMCOUNT, 0, 0));
...
void DockingCont::SelectTab(int iTab)
{
	if (iTab != -1)
	{
		...
		for (int iItem = 0; iItem < iItemCnt; iItem++)
		{
			...
			tcItem.cchTextMax = static_cast<int>(szText.size());
